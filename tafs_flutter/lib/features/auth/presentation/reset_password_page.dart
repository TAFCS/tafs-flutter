...
class _ResetPasswordPageState extends State<ResetPasswordPage> {
  ...
  Future<void> _handleResetSuccess(BuildContext context) async {
    await InjectionContainer.savedCredentialsService.clear(
      isStaff: widget.isStaff,
    );
    if (!context.mounted) return;

    if (widget.fromLoggedInSession) {
    ...
    return BlocListener<AuthBloc, AuthState>(
      ...
      listener: (context, state) async {
        if (state is ResetPasswordSuccess) {
          await _handleResetSuccess(context);
        ...
        builder: (context, state) {
          ...
          return PopScope(
            onPopInvokedWithResult: (didPop, _) {
              if (didPop && !widget.fromLoggedInSession) {
                context.read<AuthBloc>().add(
